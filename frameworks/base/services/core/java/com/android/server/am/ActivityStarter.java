...
//7
    private int startActivity(IApplicationThread caller, Intent intent, Intent ephemeralIntent,
        ...
        ProcessRecord callerApp = null; //获取调用者的进程记录对象
        if (caller != null) {
        ...
        if (resultTo != null) {
            sourceRecord = mSupervisor.isInAnyStackLocked(resultTo);//获取调用者所在的Activity
        ...
        if ((launchFlags & Intent.FLAG_ACTIVITY_FORWARD_RESULT) != 0 && sourceRecord != null) {
            ...
            if (requestCode >= 0) { //activity执行结果的返回由源Activity转换到新Activity, 不需要返回结果则不会进入该分支
        ...
        if (err == ActivityManager.START_SUCCESS && intent.getComponent() == null) {
            ...
            err = ActivityManager.START_INTENT_NOT_RESOLVED;//从Intent中无法找到相应的Component
        }

        if (err == ActivityManager.START_SUCCESS && aInfo == null) {
            // We couldn't find the specific class specified in the Intent.
            // Also the end of the line.
            err = ActivityManager.START_CLASS_NOT_FOUND;//从Intent中无法找到相应的ActivityInfo
        }
        ...
        if (mService.mController != null) {
            try {// ActivityController不为空的情况，比如monkey测试过程
        ...
        final ActivityStack stack = mSupervisor.mFocusedStack; // 将mFocusedStack赋予当前stack

        // If we are starting an activity that is not from the same uid as the currently resumed
        // one, check whether app switches are allowed.
        if (voiceSession == null && (stack.getResumedActivity() == null
                || stack.getResumedActivity().info.applicationInfo.uid != realCallingUid)) {
            if (!mService.checkAppSwitchAllowedLocked(callingPid, callingUid,  // 前台stack还没有resume状态的Activity时, 则检查app切换是否允许
                    realCallingPid, realCallingUid, "Activity start")) {
                mController.addPendingActivityLaunch(new PendingActivityLaunch(r,
                        sourceRecord, startFlags, stack, callerApp));  // 当不允许切换,则把要启动的Activity添加到mPendingActivityLaunches对象, 并且直接返回
                ActivityOptions.abort(checkedOptions);
                return ActivityManager.START_SWITCHES_CANCELED;
            }
        }

        if (mService.mDidAppSwitch) { //从上次禁止app切换以来,这是第二次允许app切换,因此将允许切换时间设置为0,则表示可以任意切换app
        ...
        mController.doPendingActivityLaunches(false);//处理 pendind Activity的启动, 这些Activity是由于app switch禁用从而被hold的等待启动activity
    ...
    private int startActivityMayWait(IApplicationThread caller, int callingUid,
        ...
        final Intent ephemeralIntent = new Intent(intent);//创建新的Intent对象，即便intent被修改也不受影响
        ...
        ActivityInfo aInfo = mSupervisor.resolveActivity(intent, rInfo, startFlags, profilerInfo);//收集Intent所指向的Activity信息, 当存在多个可供选择的Activity,则直接向用户弹出resolveActivity

        synchronized (mService) {
            final ActivityStack stack = mSupervisor.mFocusedStack;//获取stack
            stack.mConfigWillChange = globalConfig != null
                    && mService.getGlobalConfiguration().diff(globalConfig) != 0;
            if (DEBUG_CONFIGURATION) Slog.v(TAG_CONFIGURATION,
                    "Starting activity when config will change = " + stack.mConfigWillChange);

            final long origId = Binder.clearCallingIdentity();//权限控制
            ...
            Binder.restoreCallingIdentity(origId);//权限控制
...
//8
    private int startActivity(final ActivityRecord r, ActivityRecord sourceRecord,
                IVoiceInteractionSession voiceSession, IVoiceInteractor voiceInteractor,
                int startFlags, boolean doResume, ActivityOptions options, TaskRecord inTask,
                ActivityRecord[] outActivity) {// sourceRecord是指调用者， r是指本次将要启动的Activity
    ...
    // Note: This method should only be called from {@link startActivity}.//9
    private int startActivityUnchecked(final ActivityRecord r, ActivityRecord sourceRecord,
        ...
        if (reusedActivity != null) {
            ...
            if ((mLaunchFlags & FLAG_ACTIVITY_CLEAR_TOP) != 0
                ...
                if (top != null) {
                    ...
                    deliverNewIntent(top); //触发onNewIntent()
                }
            }
        ...
        if (mStartActivity.resultTo == null && mInTask == null && !mAddingToTask
            ...
            newTask = true;
            result = setTaskFromReuseOrCreateNewTask(taskToAffiliate, topStack);//新创建出一个Task
		...
		//处理新增Task和启动动画的问题 将处理好的task相关参数，传递给WindowManager为task的切换.
        mTargetStack.startActivityLocked(mStartActivity, topFocused, newTask, mKeepCurTransition,
                mOptions);
        if (mDoResume) { //处理完启动栈任务栈的问题后，准备执行发起者的Resume状态了
    ...
    private void setInitialState(ActivityRecord r, ActivityOptions options, TaskRecord inTask,
        ...
        if ((startFlags & START_FLAG_ONLY_IF_NEEDED) != 0) {
            ...
            if (!checkedCaller.realActivity.equals(r.realActivity)) {
                ...
                mStartFlags &= ~START_FLAG_ONLY_IF_NEEDED; //调用者 与将要启动的Activity不相同时，进入该分支。
            }
        }
