...
class Service::OptionParserMap : public KeywordMap<OptionParser> {
  ...
  private:
    const Map& map() const override;//定义
};

const Service::OptionParserMap::Map& Service::OptionParserMap::map() const {//实现
...
Result<Success> Service::ParseLine(const std::vector<std::string>& args) {
    ...
    auto parser = parser_map.FindFunction(args);//内部调用map方法获取对应解析器 对应L721
...
Result<Success> Service::Start() {
    ...
    if (pid == 0) {//在子进程中
        ...
        if (!ExpandArgsAndExecv(args_)) {//启动app_main.cpp的main函数
