...
class ActivityStack<T extends StackWindowController> extends ConfigurationContainer
    ...
    private boolean resumeTopActivityInnerLocked(ActivityRecord prev, ActivityOptions options) {
        ...
        final ActivityRecord next = topRunningActivityLocked(true /* focusableOnly */); // 我们的TestActivity已经入栈了，所以next就是我们的TestActivity了
        ...
        // If the top activity is the resumed one, nothing to do.// 如果我们要跳转的页面已经是resume状态，那么直接通知Wms执行Activity的过渡
        ...
        if (shouldSleepOrShutDownActivities() // 判断一下是否已经进入睡眠状态，如果进入睡眠状态Activity也会进入Pause状态
        ...
        if (mResumedActivity != null) {
            ...
            pausing |= startPausingLocked(userLeaving, false, next, false);//上一个Activity执行pause
        }
        ...
        if (prev != null) {
            if (prev.finishing) {
                ...
                if (mStackSupervisor.mNoAnimActivities.contains(next)) {
                    ...
                    mWindowManager.prepareAppTransition(prev.getTask() == next.getTask()//准备启动
            ...
            synchronized(mWindowManager.getWindowManagerLock()) {
                ...
                try {
                    final ClientTransaction transaction = ClientTransaction.obtain(next.app.thread,
                            next.appToken);
                    // Deliver all pending results.  // 判断是否需要触发onActivityResult函数
                    ...
                    if (next.newIntents != null) { //是否需要触发onNewIntent函数
